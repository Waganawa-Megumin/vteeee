 ...
 * returns an error string (not a fabricated report) when Claude is unavailable. Long reports are kept
 * complete via assistant-prefill continuation (see generateClaudeReport).
 */
export async function assessCampaign(
  digest: unknown,
  env: ProxyEnv,
): Promise<{ text: string; model?: string; error?: string; truncated?: boolean }> {
  return generateClaudeReport(ASSESS_SYSTEM, digest, env, {
    missingKeyError:
