...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === '/api/monitor-assessments' && request.method === 'GET') {
        if (!checkAccess(auth, proxy)) return json({ error: 'unauthorized' }, 401);
        return json(await getSharedMonitorAssessments(store));
      }
      if (url.pathname === '/api/monitor-assessments' && request.method === 'PUT') {
        if (origin && !originAllowed(origin, allowed)) return json({ error: 'origin not allowed' }, 403);
        if (!checkAccess(auth, proxy)) return json({ error: 'unauthorized' }, 401);
