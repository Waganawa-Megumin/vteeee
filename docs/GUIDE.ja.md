...
| **Shodan** | OSINT（IP限定） | 任意 | `SHODAN_API_KEY` | 開放ポート・稼働サービス・既知CVE・組織/ISP/OS・ホスト名・タグ。詳細で **📡 Live ports**（InternetDB/再スキャン）。**☆ Monitor** で Shodan ネットワークアラートに登録→ヘッダー **📡 Monitor** ページで監視IP＋vteeee エンリッチ結果を翌日以降も確認（Monitorは membership+ 必要） |
