...
**IOC種別で引き分け**：ドメイン→VT＋DomainTools(Enrich)＋DNSLytics(HostingHistory) ／ IP→VT＋Shodan＋DNSLytics(IPInfo)＋DomainTools(逆引き) ／ URL→ホスト名をドメイン扱い ／ ハッシュ→VTのみ。**Intel 471 と CYFIRMA は全種別**に付与。各連携は Settings で個別ON/OFF可。DomainToolsは従量課金・Investigateは低レート制限のため、`DOMAINTOOLS_RPM`(既定30)/`DNSLYTICS_RPM`(既定60)/`INTEL471_RPM`(既定60)/`CYFIRMA_RPM`(既定30)で調整、`DNSLYTICS_BASE_URL`・`CYFIRMA_BASE_URL`でエンドポイント変更可。
...
| `SHODAN_RPM` / `DOMAINTOOLS_RPM` / `DNSLYTICS_RPM` / `INTEL471_RPM` / `CYFIRMA_RPM` | プロキシ | △ | 各連携の毎分上限(既定 60/30/60/60/30) |
