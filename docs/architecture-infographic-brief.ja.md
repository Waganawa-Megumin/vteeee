...
- **面(plane)の区別（最重要）**：この輪は丸ごと**認証済みの“管理面 / control plane”**。①の人↔Claude Code は **Claude にログインした中**の操作、GitHub・Cloudflare も各コンソール／トークンで認証済み。→ **この輪に『公開ゾーン(鍵なし)』は無い**。「公開ゾーン(🔓)／保護ゾーン(🔑)」の境界は **§4 のランタイム面だけ**に描く（**ループに重ねない**）。
...
## 4. データ面（ランタイム）＝「公開ゾーン／保護ゾーン」はここだけ

デプロイ後、**アナリストがアプリを使う面**。「公開ゾーン(🔓鍵なし)／保護ゾーン(🔑鍵あり)」の信頼境界は **この面だけ**に描く（§3 の DevSecOps ループには重ねない）。「実行時に鍵を使うのは Cloudflare 側だけ」。

```mermaid
flowchart LR
  subgraph PUB["🔓 公開ゾーン（鍵なし）"]
    U["🌐 アナリストのブラウザ<br/>github.io アプリ・access token のみ"]
    P["📄 GitHub Pages（静的配信）"]
  end
  subgraph PROT["🔑 保護ゾーン（鍵あり）"]
    W["☁️ Cloudflare Worker<br/>全APIキー保持・レート制御"]
    V["🛰 外部API<br/>VirusTotal/Shodan/MaxMind…"]
  end
  P -. アプリ配信 .-> U
  U -->|"① /api/enrich ＋access token"| W
  W -->|"② 代理呼出(x-apikey等)"| V
  V -->|"③ 結果"| W -->|"④ NDJSON 逐次"| U
```

> ⚠ この「公開／保護」は**ランタイム利用者(アナリスト)のブラウザ**の話。**開発者が Claude Code を操作する面(§3)とは別**で、そちらは丸ごと認証済み（非公開）です。
...
- **構図**：上＝ヒーロー帯（3社）／中央＝DevSecOpsの輪（**認証済み管理面**・主役・最大）／下＝ランタイム面（**公開/保護ゾーン**）。**視線は上→中→下**。
- **信頼境界(公開/保護)は“下段のランタイム面の中だけ”**に置く。**上・中の DevSecOps 面には引かない**（あそこは丸ごと認証済み）。
...
- **Don’t**：**`git push` を“人の起点”として描かない**（起点は人の指示、push は Claude Code の一手）／**公開/保護ゾーンの境界を DevSecOps ループに重ねない**（あれは §4 ランタイム面だけの境界）／**①指示を「公開ゾーン(鍵なし)」に置かない**（認証済み Claude セッション内の操作）／Sec を単独の1工程にしない（横串で）／ブラウザから外部APIへ直線を引かない（誤解の元＝この図が否定したい絵）／鍵アイコンをブラウザ側に置かない／専門用語を注釈なしで多用しない。
