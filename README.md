...
- **Shodan Monitor watchlist** — ☆ Monitor an IP to register it as a **Shodan network alert** (server-side, so Shodan keeps watching it for changes) — and vteeee saves that IP's **enrichment snapshot** next to it. Reopen the **📡 Monitor** page the next day and the intel is still there (verdict · ports · geo · AbuseIPDB score · Recorded Future risk), with re-enrich / open-detail / remove per IP. Not just a raw Shodan monitor — your triaged intel travels with the watchlist. Uses the Shodan key (membership+); no data leaves the proxy.
...
- **Shodan Monitor ウォッチリスト**：IP詳細で **☆ Monitor** を押すと、その IP を **Shodan のネットワークアラート（サーバ側）** に登録し、Shodan が変化を監視し続けます。さらに vteeee が**その IP のエンリッチ結果スナップショット**も一緒に保存するので、**📡 Monitor ページ**を翌日開いても intel（判定・ポート・地域・AbuseIPDBスコア・RFリスク）ごと確認可能。IPごとに再エンリッチ／詳細を開く／削除。単なる Shodan 監視ではなく、**トリアージした intel が付いて回る**のがポイント（Shodanキー=membership+ 使用・データはプロキシ内）。
