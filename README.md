...
| **DNSLytics** | Domain/IP intel | optional | `DNSLYTICS_API_KEY` | **IPs** (IPInfo): ASN · org/ISP · network · reverse DNS · hosted-domain count. **domains** (DomainInfo): registrar · NS · MX · provider · popularity |
| **Claude (Anthropic)** | Smart-parse | optional | `ANTHROPIC_API_KEY` | extract IOCs from free-form report prose |

**Enrichment routes by IOC type:** domain → VT + DomainTools (Enrich) + DNSLytics · IP → VT + Shodan + DNSLytics + DomainTools (reverse) · URL → host treated as a domain · hash → VT only. Each optional service toggles independently in **Settings**.
