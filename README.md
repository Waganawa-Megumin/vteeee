...
| **urlscan.io** | Live capture — URL/domain (on demand) | optional | `URLSCAN_API_KEY` | **"🎣 魚拓" button**: submits the URL/domain to urlscan.io's **sandbox** (the visit originates there, not from your proxy/egress) and returns a **screenshot** · final URL · resolved IP · server ASN/country · HTTP status · **malicious verdict/score** · impersonated brands · every contacted domain/IP · link to the urlscan result. Unlisted by default (visibility in Settings) |

**Enrichment routes by IOC type:** domain → VT + DomainTools (Enrich) + DNSLytics (HostingHistory) · IP → VT + Shodan + MaxMind (geo/map) + DNSLytics (IPInfo) + DomainTools (reverse) · URL → host treated as a domain · hash → VT + ThreatVision (sample attribution). **Intel 471, CYFIRMA, ThreatVision and Recorded Future apply to every type.** **On-demand live capture (detail panel, not part of batch enrich):** URL/domain → **urlscan.io 魚拓** · IP → **📡 Live ports** (InternetDB / Shodan re-scan). Each optional service toggles independently in **Settings**.
